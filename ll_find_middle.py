...
class LinkedList:
    ...
    def find_middle_node(self):
        ...
        # The loop continues as long as the fast pointer is not None and the fast.next pointer is also not None (ensures fast can  move two steps)
        while fast and fast.next:
            # Move the slow pointer one node forward (one step at a time)
            slow = slow.next

            # Move the fast pointer two nodes forward (two steps at a time)
            fast = fast.next.next

        # When the loop ends, the fast pointer has reached the end
        # The slow pointer will be at the middle node of the linked     list
        # or the first node of the second half if the list has an even  number of nodes
